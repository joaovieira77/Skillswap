...
app.post("/matches", async (req, res) => {
  ...
  if (!userId || !skillFrom || !skillTo) {
    return res.status(400).json({ error: "Dados incompletos para criar swap" });
  }
  ...
  createdAt: new Date(),
    ...
    res.json({ message: "Swap criado com sucesso", id: result.insertedId });
  } catch (err) {
    console.error("Erro ao criar swap:", err.message);
    res.status(500).json({ error: "Erro ao criar swap" });
  }
});
...
app.post("/api/match-requests", async (req, res) => {
  ...
  try {
    ...
    if (swap.user1.toString() === user2) {
      return res.status(400).json({ error: "Não podes enviar pedido para o teu próprio swap" });
    }
    ...
    if (existing) {
      return res.status(409).json({ error: "Já enviaste um pedido para este swap" });
    }
    ...
    res.json({ message: "Pedido de match enviado", id: result.insertedId });
  } catch (err) {
    console.error("Erro ao criar pedido de match:", err.message);
    res.status(500).json({ error: "Erro ao criar pedido de match" });
  }
});
