...
export default function SkillSwaps() {
  ...
  useEffect(() => {
    const fetchSwaps = async () => {
      try {
        ...
        const res = await fetch(`http://localhost:3007/api/swaps/${encodedSkill}`);
  ...
  const handleSendRequest = async (swapId) => {
    try {
      const res = await fetch("http://localhost:3007/api/match-requests", {
        method: "POST",
      ...
      if (res.ok) {
        alert("Pedido de match enviado com sucesso!");
  ...
  return (
    <div className="min-h-screen bg-white text-gray-800 p-6">
      ...
      {loading ? (
        <p className="text-gray-500">A carregar swaps...</p>
      ) : swaps.length === 0 ? (
