...
export default function RequestsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const reqRes = await fetch(`http://localhost:3007/api/match-requests/${userId}`);
        const reqData = await reqRes.json();
        setRequests(reqData);

        const swapRes = await fetch("http://localhost:3007/api/all-swaps");
        ...
        const userRes = await fetch("http://localhost:3007/api/all-users");
  ...
  const handleAccept = async (requestId) => {
    try {
      await fetch(`http://localhost:3007/api/match-requests/${requestId}/accept`, {
        method: "POST",
      });
  ...
  const handleReject = async (requestId) => {
    try {
      await fetch(`http://localhost:3007/api/match-requests/${requestId}/reject`, {
        method: "POST",
      });
      setRequests((prev) => prev.filter((r) => r._id !== requestId));
    } catch (err) {
