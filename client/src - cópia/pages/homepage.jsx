...
export default function HomePage() {
    ...
    const fetchAll = async () => {
      try {
        const [userRes, skillsRes, swapsRes] = await Promise.all([
          ...
          fetch("http://localhost:3007/api/skills"),
          fetch("http://localhost:3007/api/all-swaps"),
        ]);
  ...
  const handleDeleteSwap = async (swapId) => {
    ...
    try {
      const res = await fetch(`http://localhost:3007/api/swaps/${swapId}`, {
        method: "DELETE",
      });
  ...
  return (
    ...
    <header className="flex items-center justify-center py-4 border-b shadow-md">
      ...
      <button
        ...
        className="fixed bottom-20 right-4 bg-blue-600 text-white rounded-full w-14 h-14 flex items-center justify-center text-3xl shadow-lg hover:bg-blue-700 transition"
