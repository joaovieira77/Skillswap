...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        const res = await fetch(`http://localhost:3007/users/${userId}`);
    ...
    const fetchSkills = async () => {
      try {
        const res = await fetch("http://localhost:3007/api/skills");
        const data = await res.json();
        setAllSkills(data);
      } catch (err) {
...
const handleSave = async () => {
  ...
  try {
    const res = await fetch("http://localhost:3007/api/users/update-profile", {
      method: "POST",
...
{/* 
          ...
          {!editing && (
            <button
              className="px-4 py-2 mt-2 bg-blue-600 text-white rounded-full hover:bg-blue-700 transition"
